// Caller — startCallOrchestration helper.
// Copied verbatim from src/features/smsv2/lib/startCallOrchestration.ts —
// pure async function (no React, no smsv2 imports), so it ports cleanly.
// Wires the manual "Call" button to the real Twilio dial flow:
//   1. Validate phone (no number → no call).
//   2. POST wk-calls-create — returns { call_id } and an `allowed` flag
//      that doubles as the spend / killswitch gate.
//   3. dial(phone, { CallId, ContactId }) — runs Twilio Voice SDK
//      device.connect() with our minted UUID baked into the params.
//   4. On success, return { ok: true, callId, twilioCall }.
