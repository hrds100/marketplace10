// CallerLoginPage — agent sign-in at /caller/login.
//
// Email + password only — the same flow CRM uses today, just rebranded
// to "Caller" and pointed at the new /caller/* routes. CallerGuard
// stamps location.state.from = the path the agent tried to open.
...
export default function CallerLoginPage() {
  ...
  const redirectTo = (location.state as { from?: string } | null)?.from ?? '/caller/inbox';
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#F3F3EE] px-4">
      <div className="w-full max-w-[440px]">
        <div className="text-center mb-6">
          ...
          <h1 className="text-[22px] font-bold text-[#0A0A0A] tracking-tight">Caller agent sign-in</h1>
        ...
        >
          ...
          <div className="text-[11px] text-[#737373] text-center pt-2">
            Forgot your password? Ask your admin to reset it from Caller → Settings → Agents.
          </div>
        </form>
