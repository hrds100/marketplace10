// Caller — useCurrentAgent hook.
// Ported from src/features/smsv2/hooks/useCurrentAgent.ts. Joins
// profiles + wk_voice_agent_limits + today's wk_calls so the status
// bar / dialer / live-call screen can read the signed-in agent's
// identity, presence, daily call counts, and spend in one place.
// Polls every 30s + realtime UPDATE on wk_voice_agent_limits + INSERT
// /UPDATE on wk_calls (filtered to this agent) for instant refresh.
  ...
  useEffect(() => {
    ...
    void (async () => {
      ...
      limitChannel = supabase
        .channel(`caller-current-agent-${uid}`)
        .on(
          // eslint-disable-next-line @typescript-eslint/no-explicit-any
