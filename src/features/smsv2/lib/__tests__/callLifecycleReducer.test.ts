...
// ──────────────────────────────────────────────────────────────────────
// PR 150 (Hugo 2026-04-29): new reducer events for the dialer rebuild.
// Tests written FIRST (TDD). Implementations land in the reducer below.
// ──────────────────────────────────────────────────────────────────────

describe('PR 150 — OUTCOME_FAILED', () => {
  it('outcome_submitting → outcome_done (no trap) on OUTCOME_FAILED', () => {
    let s = dial();
    ...
    s = callLifecycleReducer(s, {
      type: 'OUTCOME_FAILED',
      message: 'wk-outcome-apply 500',
    });
    expect(s.callPhase).toBe('outcome_done');
  });

  it('OUTCOME_FAILED outside outcome_submitting is a no-op', () => {
    const s = dial();
...
describe('PR 150 — SKIP_REQUESTED', () => {
  it('stopped_waiting_outcome → outcome_done on SKIP_REQUESTED', () => {
    let s = dial();
    s = callLifecycleReducer(s, { type: 'CALL_ENDED', reason: 'user_hangup' });
    expect(s.callPhase).toBe('stopped_waiting_outcome');
    s = callLifecycleReducer(s, { type: 'SKIP_REQUESTED' });
    expect(s.callPhase).toBe('outcome_done');
  });

  it('error_waiting_outcome → outcome_done on SKIP_REQUESTED', () => {
    let s = dial();
    s = callLifecycleReducer(s, {
      type: 'CALL_ERROR',
  ...
  it('SKIP_REQUESTED outside *_waiting_outcome is a no-op', () => {
    const s = dial();
    const after = callLifecycleReducer(s, { type: 'SKIP_REQUESTED' });
    expect(after).toBe(s);
  });
});

describe('PR 150 — NEXT_CALL_EMPTY', () => {
  it('NEXT_CALL_EMPTY in outcome_done sets the sticky banner; stays in outcome_done', () => {
    let s = dial();
    s = callLifecycleReducer(s, { type: 'CALL_ENDED', reason: 'user_hangup' });
    s = callLifecycleReducer(s, { type: 'OUTCOME_PICKED', columnId: 'col-1' });
    s = callLifecycleReducer(s, { type: 'OUTCOME_RESOLVED' });
    expect(s.callPhase).toBe('outcome_done');
    expect(s.noNewLeadsBanner).toBe(false);
    s = callLifecycleReducer(s, {
      type: 'NEXT_CALL_EMPTY',
      skippedAlreadyDialed: 2,
    });
    expect(s.callPhase).toBe('outcome_done');
    expect(s.noNewLeadsBanner).toBe(true);
  });

  it('NEXT_CALL_EMPTY outside outcome_done is a no-op', () => {
    const s = dial();
    const after = callLifecycleReducer(s, {
      type: 'NEXT_CALL_EMPTY',
  ...
  it('starting a new call clears noNewLeadsBanner', () => {
    let s = dial();
    s = callLifecycleReducer(s, { type: 'CALL_ENDED', reason: 'user_hangup' });
    s = callLifecycleReducer(s, { type: 'OUTCOME_PICKED', columnId: 'col-1' });
    s = callLifecycleReducer(s, { type: 'OUTCOME_RESOLVED' });
...
describe('PR 150 — PAUSE_REQUESTED / RESUME_REQUESTED', () => {
  it('PAUSE_REQUESTED from outcome_done while idle (no live call) → callPhase=paused', () => {
    let s = dial();
    s = callLifecycleReducer(s, { type: 'CALL_ENDED', reason: 'user_hangup' });
    s = callLifecycleReducer(s, { type: 'OUTCOME_PICKED', columnId: 'col-1' });
    s = callLifecycleReducer(s, { type: 'OUTCOME_RESOLVED' });
    s = callLifecycleReducer(s, { type: 'PAUSE_REQUESTED' });
    expect(s.callPhase).toBe('paused');
    expect(s.sessionPaused).toBe(true);
  });

  it('PAUSE_REQUESTED during in_call sets sessionPaused but keeps callPhase live', () => {
    let s = dial();
    s = callLifecycleReducer(s, { type: 'CALL_ACCEPTED', startedAt: 1 });
    s = callLifecycleReducer(s, { type: 'PAUSE_REQUESTED' });
  ...
  it('PAUSE_REQUESTED cancels any armed pacing timer', () => {
    let s = dial();
    s = callLifecycleReducer(s, { type: 'CALL_ENDED', reason: 'user_hangup' });
    s = callLifecycleReducer(s, { type: 'OUTCOME_PICKED', columnId: 'col-1' });
    s = callLifecycleReducer(s, { type: 'OUTCOME_RESOLVED' });
    ...
    expect(s.pendingNextCall).toBe('armed');
    expect(s.pacingDeadlineMs).not.toBeNull();
    s = callLifecycleReducer(s, { type: 'PAUSE_REQUESTED' });
    expect(s.pendingNextCall).toBe('idle');
    expect(s.pacingDeadlineMs).toBeNull();
  });

  it('RESUME_REQUESTED from callPhase=paused → outcome_done', () => {
    let s = dial();
    s = callLifecycleReducer(s, { type: 'CALL_ENDED', reason: 'user_hangup' });
    s = callLifecycleReducer(s, { type: 'OUTCOME_PICKED', columnId: 'col-1' });
    s = callLifecycleReducer(s, { type: 'OUTCOME_RESOLVED' });
    s = callLifecycleReducer(s, { type: 'PAUSE_REQUESTED' });
    expect(s.callPhase).toBe('paused');
    s = callLifecycleReducer(s, { type: 'RESUME_REQUESTED' });
    expect(s.callPhase).toBe('outcome_done');
    expect(s.sessionPaused).toBe(false);
  });

  it('RESUME_REQUESTED while sessionPaused mid-call clears the flag, keeps callPhase', () => {
    let s = dial();
    s = callLifecycleReducer(s, { type: 'CALL_ACCEPTED', startedAt: 1 });
    s = callLifecycleReducer(s, { type: 'PAUSE_REQUESTED' });
    expect(s.sessionPaused).toBe(true);
  ...
  it('PAUSE_REQUESTED is idempotent — second call leaves state unchanged', () => {
    let s = dial();
    s = callLifecycleReducer(s, { type: 'PAUSE_REQUESTED' });
    const ref = s;
    s = callLifecycleReducer(s, { type: 'PAUSE_REQUESTED' });
  ...
  it('RESUME_REQUESTED while not paused is a no-op', () => {
    const s = dial();
    const after = callLifecycleReducer(s, { type: 'RESUME_REQUESTED' });
    expect(after).toBe(s);
  });
});

describe('PR 150 — PACING_ARMED / PACING_CANCELLED / PACING_DEADLINE_TICK', () => {
  function reachOutcomeDone(): CallLifecycleState {
    let s = dial();
    s = callLifecycleReducer(s, { type: 'CALL_ENDED', reason: 'user_hangup' });
    s = callLifecycleReducer(s, { type: 'OUTCOME_PICKED', columnId: 'col-1' });
    s = callLifecycleReducer(s, { type: 'OUTCOME_RESOLVED' });
  ...
  it('PACING_ARMED in outcome_done sets pendingNextCall=armed + deadline', () => {
    let s = reachOutcomeDone();
    const deadline = Date.now() + 5000;
    s = callLifecycleReducer(s, { type: 'PACING_ARMED', deadlineMs: deadline });
    expect(s.pendingNextCall).toBe('armed');
    expect(s.pacingDeadlineMs).toBe(deadline);
  });

  it('PACING_ARMED outside outcome_done is a no-op (defensive)', () => {
    const s = dial();
    const after = callLifecycleReducer(s, {
      type: 'PACING_ARMED',
  ...
  it('PACING_CANCELLED clears pendingNextCall + deadline', () => {
    let s = reachOutcomeDone();
    s = callLifecycleReducer(s, {
      type: 'PACING_ARMED',
      deadlineMs: Date.now() + 5000,
    });
    s = callLifecycleReducer(s, { type: 'PACING_CANCELLED' });
    expect(s.pendingNextCall).toBe('idle');
    expect(s.pacingDeadlineMs).toBeNull();
  });

  it('PACING_DEADLINE_TICK transitions armed → cooling_down (UI shows "Resolving next…")', () => {
    let s = reachOutcomeDone();
    s = callLifecycleReducer(s, {
      type: 'PACING_ARMED',
      deadlineMs: Date.now() + 5000,
    });
    s = callLifecycleReducer(s, { type: 'PACING_DEADLINE_TICK' });
    expect(s.pendingNextCall).toBe('cooling_down');
    expect(s.callPhase).toBe('outcome_done'); // context dispatches NEXT_CALL_REQUESTED next
  });

  it('PACING_DEADLINE_TICK from idle pendingNextCall is a no-op', () => {
    const s = reachOutcomeDone();
    const after = callLifecycleReducer(s, { type: 'PACING_DEADLINE_TICK' });
    expect(after).toBe(s);
  });

  it('Starting a new call clears pacing fields', () => {
    let s = reachOutcomeDone();
    s = callLifecycleReducer(s, {
      type: 'PACING_ARMED',
      deadlineMs: Date.now() + 5000,
