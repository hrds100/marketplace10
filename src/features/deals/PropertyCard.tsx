  ...
  return (
    <div data-feature="DEALS__PROPERTY_CARD" className="bg-card border border-border rounded-2xl overflow-hidden card-hover flex flex-col">
      ...
      <div className="p-3.5 pt-3 flex flex-col flex-1">
        ...
        {/* Action row: [ Visit Listing ] [ Email icon ] [ WhatsApp icon ] */}
        {/* Email icon only shows if lister has email, WhatsApp only if has phone — auto-adjusts */}
        <div className="flex gap-2 mt-3">
          ...
          <button data-feature="DEALS__PROPERTY_CARD_EMAIL" onClick={forceSignUp ? handleAction : handleEmail}
            className="h-[42px] w-[48px] flex-shrink-0 rounded-xl inline-flex items-center justify-center transition-all hover:brightness-[0.96]"
            style={{ backgroundColor: '#E8F0EF' }}>
            <Mail className="w-[18px] h-[18px]" strokeWidth={1.8} style={{ color: '#2D6A5F' }} />
          </button>
          <button data-feature="DEALS__PROPERTY_CARD_WHATSAPP" onClick={forceSignUp ? handleAction : handleWhatsApp}
            className="h-[42px] w-[48px] flex-shrink-0 rounded-xl inline-flex items-center justify-center transition-all hover:brightness-[0.96]"
