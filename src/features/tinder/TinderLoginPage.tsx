// TinderLoginPage — dedicated agent sign-in at /tinder/login.
//
// Mirrors src/features/smsv2/pages/CrmLoginPage.tsx so /tinder gets the same
// purpose-built login experience as /crm. Just email + password; no social
// login, no account-creation links. Whatever the admin typed when inviting
// the agent in /tinder/agents (or /crm/settings → Agents — same backend) is
// what they paste in here.
//
// On success: bounce to /tinder (or whatever the location.state.from path
// the user originally tried to open). The ProtectedRoute in TinderLayout
...
export default function TinderLoginPage() {
  ...
  // ProtectedRoute stamps location.state.from = the path the user tried to
  // open. Fall back to /tinder so a clean visit to /tinder/login still
  // lands somewhere useful.
  const redirectTo = (location.state as { from?: string } | null)?.from ?? "/tinder";
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#F3F3EE] px-4">
      <div className="w-full max-w-[440px]">
        <div className="text-center mb-6">
          ...
          <h1 className="text-[22px] font-bold text-[#0A0A0A] tracking-tight">Tinder sign-in</h1>
        ...
        >
          ...
          <div className="text-[11px] text-[#737373] text-center pt-2">
            Forgot your password? Ask Hugo to reset it from{" "}
            <Link to="/tinder/agents" className="text-[#1E9A80] hover:underline">/tinder/agents</Link>.
          </div>
        </form>

        <div className="text-center mt-5 text-[12px] text-[#737373]">
          Not a Tinder agent?{" "}
          <Link to="/crm/login" className="text-[#1E9A80] font-semibold hover:underline">
            CRM sign-in
          </Link>{" "}
          ·{" "}
