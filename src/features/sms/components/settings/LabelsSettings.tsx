...
interface LabelsSettingsProps {
  labels: SmsLabel[];
  onAdd: (label: SmsLabel) => void;
  onEdit: (label: SmsLabel) => void;
  onDelete: (id: string) => void;
}

const COLOUR_OPTIONS = ['#1E9A80', '#EF4444', '#F59E0B', '#6B7280', '#1A1A1A'];

export default function LabelsSettings({ labels, onAdd, onEdit, onDelete }: LabelsSettingsProps) {
  ...
  function startEdit(label: SmsLabel) {
  ...
  function save() {
    if (!name.trim()) return;
    if (editingId) {
      onEdit({ id: editingId, name: name.trim(), colour, position: labels.find((l) => l.id === editingId)?.position ?? 0 });
      toast.success('Label updated');
    } else {
      onAdd({ id: `lbl-${Date.now()}`, name: name.trim(), colour, position: labels.length });
  ...
  if (labels.length === 0 && !adding) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        ...
        <p className="text-[#1A1A1A] font-semibold mb-1">No labels</p>
        <p className="text-sm text-[#6B7280] mb-4">Create labels to organise your contacts</p>
          ...
          {editingId === label.id ? (
            ...
            <div className="flex items-center justify-between p-3 rounded-lg border border-[#E5E7EB] bg-white">
              <div className="flex items-center gap-2.5">
                <span className="h-3 w-3 rounded-full shrink-0" style={{ backgroundColor: label.colour }} />
                <span className="text-sm font-medium text-[#1A1A1A]">{label.name}</span>
              </div>
              <div className="flex items-center gap-1">
                <Button variant="ghost" size="sm" onClick={() => startEdit(label)} className="h-7 w-7 p-0 text-[#9CA3AF] hover:text-[#1A1A1A]">
                ...
                <Button
                  ...
                  className={cn('h-7 w-7 p-0', deleteConfirm === label.id ? 'text-[#EF4444]' : 'text-[#9CA3AF] hover:text-[#EF4444]')}
                >
