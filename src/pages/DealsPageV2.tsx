...
export default function DealsPageV2() {
  ...
  // All visible listings for map pins (featured + page)
  const mapListings = useMemo(() => [...featured, ...pageListings], [featured, pageListings]);
  ...
  return (
    <div className="flex gap-6 items-start">
      ...
      <div className="flex-1 min-w-0">
        ...
        {/* Alert */}
        {showAlert && (
          <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-3 px-4 mb-5 flex items-center gap-2.5">
            <Bell className="w-4 h-4 text-emerald-600 flex-shrink-0" />
            <span className="text-[13px] font-medium text-emerald-900">3 new deals added in Manchester today</span>
            <button className="text-[13px] text-emerald-700 font-semibold ml-1 hover:underline" onClick={handleAlertClick}>View</button>
        ...
        {featured.length > 0 && (
          <div className="mb-6">
            <span className="text-[13px] font-semibold text-foreground mb-2.5 block">⭐ Featured</span>
        ...
        <div className="flex items-center justify-between mb-4 flex-wrap gap-3">
          ...
          <span className="text-[12px] text-muted-foreground">{filtered.length + featured.length} deals</span>
        </div>

        {/* Filters */}
        <div className="flex flex-wrap gap-2 mb-4">
          <select value={city} onChange={e => { setCity(e.target.value); setPage(1); }} className="input-nfstay h-[34px] text-xs pr-8 bg-card">
            <option value="">All cities</option>
            {cities.map(c => <option key={c} value={c}>{c}</option>)}
          </select>
          <select value={type} onChange={e => { setType(e.target.value); setPage(1); }} className="input-nfstay h-[34px] text-xs pr-8 bg-card">
            <option value="">All types</option>
            {types.map(t => <option key={t} value={t}>{t}</option>)}
          </select>
          <select value={sort} onChange={e => setSort(e.target.value)} className="input-nfstay h-[34px] text-xs pr-8 bg-card ml-auto">
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-center gap-2 mt-6">
            <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1} className="text-xs text-muted-foreground hover:text-foreground disabled:opacity-40">Prev</button>
            {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => i + 1).map(p => (
              <button key={p} onClick={() => setPage(p)} className={`w-7 h-7 rounded-full text-xs font-medium ${page === p ? 'bg-foreground text-background' : 'text-muted-foreground hover:text-foreground'}`}>{p}</button>
            ))}
            {totalPages > 5 && <span className="text-muted-foreground text-xs">…</span>}
            {totalPages > 5 && (
              <button onClick={() => setPage(totalPages)} className={`w-7 h-7 rounded-full text-xs font-medium ${page === totalPages ? 'bg-foreground text-background' : 'text-muted-foreground hover:text-foreground'}`}>{totalPages}</button>
            )}
            <button onClick={() => setPage(p => Math.min(totalPages, p + 1))} disabled={page === totalPages} className="text-xs text-muted-foreground hover:text-foreground disabled:opacity-40">Next</button>
          </div>
        )}
      </div>
