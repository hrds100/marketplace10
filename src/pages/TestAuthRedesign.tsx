...
import { Eye, EyeOff, Mail, Lock, User, Phone, CheckCircle2, ArrowLeft } from 'lucide-react';
import AuthSlidePanel from '@/components/AuthSlidePanel';

/**
 * TEST PAGE — preview of the auth redesign (v2).
 * No real auth logic — purely visual.
 * Visit: /testing/auth-redesign
 *
 * Flow:
 *   Sign In: social 2×2 on top → "or sign in with email" → email+password
 *   Register Step 1: 4 stacked social buttons + "Sign up with Email" button
 *   Register Step 2a (after social): WhatsApp only
 *   Register Step 2b (after email click): full email form
 */
...
// ── Shared input ──
function AuthInput({
  icon: Icon, type = 'text', placeholder, id, hasToggle, showPassword, onToggle,
}: {
  icon: React.ElementType; type?: string; placeholder: string; id: string;
  hasToggle?: boolean; showPassword?: boolean; onToggle?: () => void;
}) {
  return (
    <div className="relative">
      ...
      />
      {hasToggle && onToggle && (
        <button type="button" onClick={onToggle} className="absolute right-2.5 top-1/2 -translate-y-1/2 bg-transparent border-none cursor-pointer p-0.5 text-[#737373] hover:text-[#0a0a0a] flex items-center">
...
// ── Social pill button ──
function SocialBtn({ icon, label, onClick, full }: { icon: React.ReactNode; label: string; onClick?: () => void; full?: boolean }) {
  return (
    <button
      onClick={onClick}
      className={`flex items-center justify-center gap-2 bg-transparent text-[#0a0a0a] border border-[#e5e5e5] rounded-full text-[15px] font-medium cursor-pointer transition-all duration-150 hover:bg-[#f5f5f5] hover:border-[#c8c8c8] ${full ? 'w-full' : 'flex-1'}`}
...
type View = 'signin' | 'register-social' | 'register-whatsapp' | 'register-email';
...
export default function TestAuthRedesign() {
  ...
  const isMainView = view === 'signin' || view === 'register-social';
        ...
        >
          ...
          <div className="flex flex-col items-center justify-center w-full max-w-[480px] flex-1">
            ...
            <div className="text-center w-full" style={{ marginBottom: 'clamp(16px, 2.5vh, 32px)' }}>
              ...
              <p className="text-base text-[#737373] text-center mt-1.5 leading-relaxed">{heading.sub}</p>
            </div>
            ...
            {isMainView && (
              <div
                ...
                style={{ height: 40, gap: 2, backgroundColor: '#f3f3ee', borderColor: '#e8e5df', padding: 2, marginBottom: 'clamp(11px, 2vh, 29px)' }}
              >
                {(['signin', 'register'] as const).map((tab) => (
                  <button
                    key={tab}
                    onClick={() => setView(tab === 'signin' ? 'signin' : 'register-social')}
                    ...
                    style={{
                      ...
                      color: activeTab === tab ? '#1b1b1b' : '#73757c',
              ...
              <div className="w-full flex flex-col" style={{ gap: 'clamp(9px, 1.8vh, 22px)' }}>
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <AuthInput icon={Lock} placeholder="Enter your password" id="si-password" hasToggle showPassword={showPassword} onToggle={() => setShowPassword(!showPassword)} />
                  </div>
                </div>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <input
                      type="checkbox" checked={rememberMe} onChange={(e) => setRememberMe(e.target.checked)} id="remember"
                  ...
                  <button className="bg-transparent border-none text-[#1e9a80] text-sm font-medium cursor-pointer p-0 hover:underline">Forgot Password?</button>
                </div>

                {/* Submit */}
                <button className="w-full rounded-lg font-medium text-white cursor-pointer transition-all duration-150 hover:opacity-90" style={{ height: 37, backgroundColor: '#1e9a80', fontSize: 16, padding: '8px 16px', border: 'none', boxShadow: '0 4px 8px -1px rgba(0,0,0,0.05)' }}>
                  Sign In
                </button>

                <p className="text-sm text-[#737373] text-center mt-2">
                  Don't have an account?{' '}
                  <button onClick={() => setView('register-social')} className="text-[#1e9a80] font-semibold bg-transparent border-none cursor-pointer p-0">Sign up</button>
                </p>
              </div>
            )}

            {/* ─── VIEW: REGISTER — Step 1 (Social + Email choice) ─── */}
            {view === 'register-social' && (
              <div className="w-full flex flex-col" style={{ gap: 'clamp(9px, 1.8vh, 22px)' }}>
                {/* Social stacked */}
                <div className="flex flex-col gap-2 w-full">
                  <SocialBtn icon={<GoogleIcon />} label="Continue with Google" full onClick={() => setView('register-whatsapp')} />
                  <SocialBtn icon={<AppleIcon />} label="Continue with Apple" full onClick={() => setView('register-whatsapp')} />
                  <SocialBtn icon={<XIcon />} label="Continue with X" full onClick={() => setView('register-whatsapp')} />
                  <SocialBtn icon={<FacebookIcon />} label="Continue with Facebook" full onClick={() => setView('register-whatsapp')} />
                </div>
                ...
                <p className="text-sm text-[#737373] text-center mt-2">
                  ...
                  <button onClick={() => setView('signin')} className="text-[#1e9a80] font-semibold bg-transparent border-none cursor-pointer p-0">Sign in</button>
                </p>
              </div>
            )}

            {/* ─── VIEW: REGISTER — Step 2a (WhatsApp only, after social) ─── */}
            {view === 'register-whatsapp' && (
              <div className="w-full flex flex-col" style={{ gap: 'clamp(9px, 1.8vh, 22px)' }}>
                <button onClick={() => setView('register-social')} className="flex items-center gap-1.5 text-sm text-[#737373] bg-transparent border-none cursor-pointer p-0 hover:text-[#0a0a0a] mb-2">
                  <ArrowLeft className="w-4 h-4" /> Back
                </button>

                <div className="mb-2">
                  <p className="text-sm text-[#737373]">
                    Signed in as <span className="font-medium text-[#0a0a0a]">user@gmail.com</span>
                  </p>
                ...
                <div className="flex flex-col gap-1">
                  ...
                  <div className="flex">
                    <div className="flex items-center gap-1.5 h-[41px] px-3 rounded-l-[10px] border border-r-0 border-[#e5e5e5] bg-white text-sm shrink-0 shadow-[0_4px_8px_-1px_rgba(0,0,0,0.05)]">
                      ...
                      <svg className="w-3 h-3 text-[#737373]" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" /></svg>
                    </div>
                    <div className="relative flex-1">
                      <Phone className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-[#737373] pointer-events-none" />
                      <input type="tel" placeholder="7863 992 555" className="w-full h-[41px] bg-white text-[#0a0a0a] border border-[#e5e5e5] rounded-r-[10px] text-sm font-normal outline-none transition-all duration-150 shadow-[0_4px_8px_-1px_rgba(0,0,0,0.05)] focus:border-[#1e9a80] focus:shadow-[0_0_0_3px_rgba(30,154,128,0.15)]" style={{ padding: '4px 12px 4px 40px' }} />
                    </div>
                  </div>
                  <p className="text-[11px] text-[#737373] mt-1">We'll send a 4-digit verification code via WhatsApp</p>
                </div>

                <button className="w-full rounded-lg font-medium text-white cursor-pointer transition-all duration-150 hover:opacity-90 flex items-center justify-center gap-2" style={{ height: 37, backgroundColor: '#1e9a80', fontSize: 16, padding: '8px 16px', border: 'none', boxShadow: '0 4px 8px -1px rgba(0,0,0,0.05)' }}>
                  <CheckCircle2 className="w-4 h-4" /> Send verification code
                </button>

                <p className="text-xs text-[#737373] text-center mt-2">
                  By continuing you agree to our{' '}
                  <span className="text-[#1e9a80] font-semibold underline cursor-pointer">Terms</span> and{' '}
                  <span className="text-[#1e9a80] font-semibold underline cursor-pointer">Privacy Policy</span>.
                </p>
              </div>
            )}

            {/* ─── VIEW: REGISTER — Step 2b (Full email form) ─── */}
            {view === 'register-email' && (
              <div className="w-full flex flex-col" style={{ gap: 'clamp(9px, 1.8vh, 22px)' }}>
                <button onClick={() => setView('register-social')} className="flex items-center gap-1.5 text-sm text-[#737373] bg-transparent border-none cursor-pointer p-0 hover:text-[#0a0a0a] mb-2">
                  <ArrowLeft className="w-4 h-4" /> Back
                ...
                <div className="flex flex-col gap-4">
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <div className="relative">
                      ...
                      <input
                        type={showPassword ? 'text' : 'password'} placeholder="Min 8 characters" value={pw} onChange={(e) => setPw(e.target.value)}
                        className="w-full h-[41px] bg-white text-[#0a0a0a] border border-[#e5e5e5] rounded-[10px] text-sm font-normal outline-none transition-all duration-150 shadow-[0_4px_8px_-1px_rgba(0,0,0,0.05)] focus:border-[#1e9a80] focus:shadow-[0_0_0_3px_rgba(30,154,128,0.15)]"
                        style={{ padding: '4px 40px 4px 40px' }}
                      />
                      <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-2.5 top-1/2 -translate-y-1/2 bg-transparent border-none cursor-pointer p-0.5 text-[#737373] hover:text-[#0a0a0a] flex items-center">
                        {showPassword ? <EyeOff className="w-5 h-5" /> : <Eye className="w-5 h-5" />}
                      </button>
                    </div>
                  ...
                  <div className="flex flex-col gap-1">
                    ...
                    <AuthInput icon={Lock} placeholder="Re-enter password" id="reg-confirm" hasToggle showPassword={showConfirm} onToggle={() => setShowConfirm(!showConfirm)} />
                  </div>
                  {/* WhatsApp */}
                  <div className="flex flex-col gap-1">
                    <label className="text-sm font-medium text-[#525252] tracking-wide">WhatsApp Number <span className="text-red-500">*</span></label>
                ...
                <label className="flex items-start gap-3 cursor-pointer">
                  <input type="checkbox" className="mt-0.5 appearance-none w-5 h-5 border border-[#e5e5e5] rounded cursor-pointer transition-all duration-150 checked:bg-[#1e9a80] checked:border-[#1e9a80] bg-white shrink-0" />
                  <span className="text-xs text-[#737373] leading-relaxed">
                    I agree to the <span className="text-[#1e9a80] font-semibold underline cursor-pointer">Terms of Service</span>
                    {' '}and <span className="text-[#1e9a80] font-semibold underline cursor-pointer">Privacy Policy</span>
