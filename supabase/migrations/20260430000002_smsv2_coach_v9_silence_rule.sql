-- ============================================================================
-- SMSV2 — Coach prompts v9 (SILENCE RULE)
--
-- Why:
--   Hugo 2026-04-30 (PR D): the live coach was generating a fresh card
--   for nearly every caller utterance — including filler ("yeah",
--   "right"), backchannel, and questions already answered by the
--   on-screen script. Result: too many coach cards, agent gets distracted
--   from the script. Hugo's call: coach should mostly point at the
--   script and only fire when the caller asks something the script /
--   knowledge base doesn't already answer.
--
--   v9 ships:
--     - SILENCE RULE block prepended to coach_script_prompt: model
--       outputs the literal marker `STAY_ON_SCRIPT` for filler /
--       acknowledgement / mid-thought / questions already covered.
--       Edge fn postProcessCoachText drops the marker → no card row.
--
--   Edge fn config tightened in the same PR (not in this migration):
--     - p_min_age_ms 250 → 700 (~3× fewer generations)
--     - temperature 0.55 → 0.4
--     - postProcessCoachText: early return null on STAY_ON_SCRIPT
--
--   Same content lives as DEFAULT_SCRIPT_PROMPT in
--   supabase/functions/wk-voice-transcription/index.ts. They MUST
--   stay in sync — change them together.
-- ============================================================================

DO $$
DECLARE
  script_v9 text := $script$You follow the NFSTAY call script. Default to script INTENT, paraphrase fresh each time. Only deviate when the caller asks a direct factual question or raises an objection.
...
OUTPUT
Return exactly one read-aloud line for the next thing the rep should say. If the SILENCE RULE applies, return only the literal marker `STAY_ON_SCRIPT` instead.$script$;
BEGIN
  -- Update the canonical singleton row.
  UPDATE wk_ai_settings
     SET coach_script_prompt = script_v9,
         updated_at = now()
   WHERE name = 'default';

  -- Future fresh installs get v9 too.
  EXECUTE format(
