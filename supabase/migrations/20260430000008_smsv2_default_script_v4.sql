-- ============================================================================
-- SMSV2 — Default call script v4 (Hugo 2026-04-26 evening)
--
-- Why:
--   Hugo on a real call: "this is not script, what to say there? come
--   on." He saw "If active: probe budget + timeline" and "If watching:
--   build interest before pitching" — both are stage directions, not
--   read-aloud lines, so the agent runs out of words mid-call.
--
--   Two issues being fixed:
--     1. v3 (PR 7) section 3 still has a directive: "If yes: continue
--        to the pitch." Replaced with a real line.
--     2. v3 Qualify branches end the conversation prematurely (only one
--        line each, no follow-up). Lengthened so the agent always has
--        the next sentence ready.
--
--   Hugo will see this update only after he clicks "Reset to default"
--   in the script-edit modal — see PR 21 Part B. The migration only
--   updates the canonical default row; personal copies (rows where
--   owner_agent_id IS NOT NULL) are not touched.
-- ============================================================================
...
- If investing: "Nice, so you already get how this works — what kind of returns are you currently seeing on your places?"
- If exploring: "Perfect, this is a simple way to get started without having to run it yourself. Want me to walk you through how it works?"

## 3. Permission to pitch
- "Great. Would it be okay if I explain quickly how our deals work?"
- If yes: "Brilliant — I'll keep it under two minutes."
...
$body$;
BEGIN
  -- Update the canonical default row.
  UPDATE wk_call_scripts
     SET body_md    = default_body,
         updated_at = now()
   WHERE is_default = true;

  -- ALTER the column DEFAULT so fresh installs pick up v4 too.
  EXECUTE format(
    'ALTER TABLE wk_call_scripts ALTER COLUMN body_md SET DEFAULT %L',
    default_body
  );

  -- Defensive: if the default row is missing, recreate it.
