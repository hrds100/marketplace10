...
contract RentTestnet is Ownable {
    /**
    ...
    uint256 private constant ONEMONTH = 15 minutes;
    ...
    /**
     * @dev Reference to the voting contract interface.
     */
    IVoting private _helperVoting;
     ...
     * @param _votingAddress The address of the voting contract.
    ...
    constructor(
        ...
        _helperVoting = IVoting(_votingAddress);
    }
    ...
    function addRent(
        ...
        uint256 _totalShares = _helperRWA.getProperty(_propertyId).totalShares;
     ...
     */
    function resetPropertyDetails(uint256 _propertyId) external onlyOwner {
        address _sender = _msgSender();
        Details memory _details = _rentDetails[_propertyId];
        ...
        _rentDetails[_propertyId] = _details;
        _helperUSDC.transfer(_sender, _rentRemaining);
        emit RentStatus(
            _sender,
            _propertyId,
            _rentRemaining,
    ...
    function withdrawRent(uint256 _propertyId) external {
        //TODO: add the claim with fiat feature here
        address _sender = _msgSender();
        uint256 _currentTime = block.timestamp;
        Details memory _details = _rentDetails[_propertyId];
        if (_details.rentRemaining == 0) revert NoRentAvailable();
        ...
        uint256 _lastVoteTime = _helperVoting.getVotingHistory(
            _sender,
            _propertyId
        );
        uint256 _diff = _currentTime - _lastVoteTime;
    ...
    function isEligibleForRent(
        ...
        if (_details.rentRemaining == 0 || _currentTime > _details.endTime)
            return false;
        ...
        uint256 _timeSinceLastVote = _currentTime - _lastVoteTime;

        if (
            _timeSinceLastVote > ONEMONTH ||
            _holdingTime > _details.startTime ||
            _lastClaimTime > _details.startTime ||
            _userShares == 0
        ) return false;
