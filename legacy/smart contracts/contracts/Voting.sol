...
contract Voting is Ownable {
    ...
    /**
     * @dev The address of the STAY token contract.
     */
    address private constant STAY = 0x7F14ce2A5df31Ad0D2BF658d3840b1F7559d3EE0;

    /**
     * @dev The address of the WBNB token contract.
     */
    address private constant WBNB = 0xbb4CdB9CBd36B01bD1cBaEBF2De08d9173bc095c;

    /**
     * @dev The address of the router contract used for token swaps.
     */
    ...
    /**
     * @dev Interface for interacting with the STAY token contract.
     */
    IERC20 private _helperStay = IERC20(STAY);

    /**
     * @dev Interface for interacting with the router contract.
     */
    ...
    /**
     * @dev Mapping to track the last voting timestamp for each voter and proposal ID.
     */
    mapping(address => mapping(uint256 => uint256)) private _votingHistory;
    ...
    function addProposal(
        ...
        _path[0] = STAY;
        _path[1] = USDC;
        uint256[] memory _amounts = _helperRouter.getAmountsIn(
            _proposalFee,
            _path
        );
        _helperStay.transferFrom(_sender, owner(), _amounts[0]);
    ...
    function vote(uint256 _proposalId, bool _inFavor) external {
        ...
        _votingHistory[_sender][_proposal.propertyId] = _currentTime;
    ...
    /**
     * @notice Allows the owner to delete an ongoing proposal.
     * @param _proposalId The ID of the proposal to delete.
     * @dev Deletes the proposal if it is in the `OnGoing` state.
     *      Reverts with `CantDeleteProposal` if the proposal is not ongoing.
     *      Emits a `ProposalStatus` event with an end time of `0` upon successful deletion.
     */
    ...
    /**
     * @notice Retrieves the last voting timestamp for a specific user and proposal.
     * @param _voter The address of the voter.
     * @param _proposalId The ID of the proposal.
     * @return The timestamp of the user's last vote on the proposal.
     */
    function getVotingHistory(
        address _voter,
        uint256 _proposalId
    ) external view returns (uint256) {
        return _votingHistory[_voter][_proposalId];
    }
