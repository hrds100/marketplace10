"use client";
import {
  AUTHOR_IMAGE_URL,
  CONTRACT_CONFIG,
  IMAGE_URL,
  WERT_BASE_VALUE,
} from "@/config";
import {
  formatNumber,
  getErrorMessage,
  getEthFrom,
  getWeiFrom,
  NotifyError,
  NotifySuccess,
  truncateAmount,
} from "@/context/helper";
import { useNfstayContext } from "@/context/NfstayContext";
import WertWidget from "@wert-io/widget-initializer";
import { useEffect, useState } from "react";
import Image from "next/image";

import Graph from "./graph";
import stay from "../images/stay.png";
import usdt from "../images/usdc.svg";
import { ethers } from "ethers";
import { signSmartContractData } from "@wert-io/widget-sc-signer";
import Claim from "@/utils/claim";
import Congratulation from "./congratulation";
import Tooltip from "@/utils/tooltip";

const Analytics = () => {
  const {
    getLpEstimation,
    connectedAddress,
    getbuyLpContract,
    getValueFromRouter,
    getFarmContract,
    getUserDetails,
    userDetails,
    assetPrices,
    handleAddToFarm,
    isFarmApprovalLoading,
    handleBuyLp,
    isBuyLpApprovalLoading,
    handleClaimRewards,
    getPairContract,
    globalLoader,
    getBalances,
    assetBalance,
    handleNetwork,
  } = useNfstayContext();

  const [currentButton, setCurrentButton] = useState("add");
  const [balance, setBalance] = useState(null);
  const [estimation, setEstimation] = useState("");
  const [amount, setAmount] = useState("");
  const [isLoading, setIsloading] = useState(false); //CRYPTO/Card payments
  const [isCardLoading, setIsCardLoading] = useState(false); //CRYPTO/Card payments
  const [loadCalculating, setLoadCalculating] = useState(false); //CRYPTO/Card payments
  const [isAddFarmLoading, setIsAddFarmloading] = useState(false);
  const [isClaimLoading, setIsClaimloading] = useState(false);
  const [isRemoveLoading, setIsRemoveLoading] = useState(false);
  const [open, setOpen] = useState(false);
  const [percentChange, setPercentChange] = useState(null);
  const [currentStep, setCurrentStep] = useState(-1);
  const [tradingVolume, setTradingVolumg] = useState(0);
  const [isSuccess, setIsSuccess] = useState(false);

  const steps = [
    "Claim STAY",
    "Approve STAY",
    "Create LP",
    "Approve LP",
    "Deposit LP",
    "Success",
  ];

  useEffect(() => {
    if (currentStep == steps.length - 1) {
      setTimeout(() => {
        setShowCongrats(true);
        setOpen(false);
      }, 800);
    }
  }, [currentStep, connectedAddress]);

  useEffect(() => {
    setCurrentStep(-1);
  }, [open]);

  const handleOpen = async () => {
    if (!connectedAddress) return NotifyError("Please connect your wallet"); 
await handleNetwork();
    setOpen(true);
  };

  const handleAddLiqudity = async (address) => {
    try {
      if (!connectedAddress) return NotifyError("Please connect your wallet"); 

      setIsAddFarmloading(true);
      await handleNetwork();

      let contract = getPairContract(CONTRACT_CONFIG.stayUsdcPair);

      const _balance = await contract.balanceOf(connectedAddress);
      await handleAddToFarm(address, getEthFrom(_balance));
    } catch (err) {
      console.log(err);
      const _msg = getErrorMessage(err);
      NotifyError(_msg);
    } finally {
      setIsAddFarmloading(false);
    }
  };

  const handleLpBuy = async (address) => {
    try {
      if (!connectedAddress) return NotifyError("Please connect your wallet"); 

      setIsloading(true);
      await handleNetwork();

      await handleBuyLp(address, amount);
      setAmount("");
      NotifySuccess("LP Bought Successfully");
    } catch (err) {
      console.log(err);
      const _msg = getErrorMessage(err);
      NotifyError(_msg);
    } finally {
      setIsloading(false);
    }
  };

  const claimRewards = async (address) => {
    try {
      setIsClaimloading(true);
      await handleNetwork();

      await handleClaimRewards(address);
      setIsSuccess(true);
    } catch (err) {
      console.log(err);
      const _msg = getErrorMessage(err);
      NotifyError(_msg);
    } finally {
      setIsClaimloading(false);
    }
  };

  const buyLpWithCard = async (_amountInUSDC) => {
    try {
      if (!connectedAddress) throw new Error("Please connect your wallet"); 
      setIsCardLoading(true);
      await handleNetwork();

      const contract = getbuyLpContract(true);

      let ABI = ["function buyLPToken(address, address, uint256)"];
      let iface = new ethers.utils.Interface(ABI);
      const sc_input_data = iface.encodeFunctionData("buyLPToken", [
        connectedAddress,
        CONTRACT_CONFIG.zeroAddress,
        getWeiFrom(_amountInUSDC),
      ]);
      const amountConversion = await getValueFromRouter(
        CONTRACT_CONFIG.WBNB,
        _amountInUSDC
      );

      const amountToBeSend = truncateAmount(getEthFrom(amountConversion));
      if (amountToBeSend === 0) {
        NotifyError("Invalid amount provided");
        return;
      }
   
      const signedData = signSmartContractData(
        {
          address: connectedAddress,
          commodity_amount: truncateAmount(
            String(amountToBeSend + WERT_BASE_VALUE)
          ),
          commodity: "BNB", // MAINNETBNB
          network: "bsc",
          // commodity: "ETH", TESTNETSEPOLIA
          // network: "sepolia",
          sc_address: CONTRACT_CONFIG.buyLp,
          sc_input_data,
        },
        process.env.NEXT_PUBLIC_WERT_PRIVATE_KEY
      );
      const icoOptions = {
        item_info: {
          author: "NFsTay.com",
          author_image_url: AUTHOR_IMAGE_URL,
          image_url: IMAGE_URL,
          name: "RWA Shares",
          category: "Buy Property Shares",
        },
      };
      const otherWidgetOptions = {
        partner_id: process.env.NEXT_PUBLIC_WERT_PARTNER_ID,
        origin: process.env.NEXT_PUBLIC_WERT_BASE_URL,
        extra: icoOptions,
      };
      const wertWidget = new WertWidget({
        ...signedData,
        ...otherWidgetOptions,
        listeners: {
          "payment-status": async (data) => {
            if (data.status === "success") {
              setIsloading(false);
              wertWidget.close();
              NotifySuccess("LP Bought Successfully");
            } else if (
              data.status === "failed" ||
              data.status === "failover" ||
              data.status === "canceled"
            ) {
              setIsloading(false);
              NotifyError("Something Went Wrong");
            }
          },
          close: () => {
            setIsloading(false);
          },
        },
      });
      wertWidget.open();
      await getUserDetails();
      setAmount("");
      NotifySuccess("LP Bought Successfully");
    } catch (e) {
      NotifyError(e.message || e.reason || "Something went wrong");
      console.log(e);
    } finally {
      setIsCardLoading(false);
    }
  };

  const handleRemoveLiquidity = async (amount) => {
    try {
      if (!connectedAddress) return NotifyError("Please connect your wallet"); 
      setIsRemoveLoading(true);
      await handleNetwork();

      const contract = getFarmContract(true);

      await contract.callStatic.unstakeLPs(getWeiFrom(amount.toString()));
      const _unstake = await contract.unstakeLPs(getWeiFrom(amount.toString()));
      await _unstake.wait();

      await getUserDetails();
      setBalance(null);
      NotifySuccess("LP Unstaked Successfully");
      await getBalances(connectedAddress);
    } catch (err) {
      console.log(err);
      const _msg = getErrorMessage(err);
      NotifyError(_msg);
    } finally {
      setIsRemoveLoading(false);
    }
  };

  const getPercentChange = async () => {
    const options = {
      method: "GET",
      headers: {
        accept: "application/json",
        "x-cg-demo-api-key": CONTRACT_CONFIG.coinGeckoApiKey,
      },
    };

    try {
      const response = await fetch(
        "https://api.coingecko.com/api/v3/coins/nfstay?localization=false&tickers=false&market_data=true&community_data=false&developer_data=false&sparkline=false",
        options
      );
      const data = await response.json();
      setPercentChange({
        price_change_24h: data?.market_data.price_change_24h,
        price_change_percentage_24h:
          data?.market_data.price_change_percentage_24h?.toFixed(2),
        lineColor:
          data?.market_data.price_change_24h < 0 ? "#ff0000" : "#20E19F",
      });
    } catch (error) {
      console.error(error);
    }
  };

  const getTradingVolume = async () => {
    const options = {
      method: "GET",
      headers: {
        accept: "application/json",
        "x-cg-demo-api-key": CONTRACT_CONFIG.coinGeckoApiKey,
      },
    };

    try {
      const response = await fetch(
        "https://api.coingecko.com/api/v3/coins/nfstay?localization=false&tickers=false&market_data=true&community_data=false&developer_data=false&sparkline=false",
        options
      );
      const data = await response.json();

      const tradingVolume24h = data?.market_data?.total_volume?.usd; // Fetch 24h trading volume in USD
      setTradingVolumg(tradingVolume24h);
    } catch (error) {
      console.error("Error fetching trading volume:", error);
    }
  };

  useEffect(() => {
    // Simulate fetching data
    const fetchPriceData = async () => {
      setTimeout(() => {
        getTradingVolume();
        getPercentChange(); // Fetch percentage change
      }, 2000); // Simulate a 2-second loading delay
    };

    fetchPriceData();
  }, []);

  useEffect(() => {
    setLoadCalculating(true);
    if (amount > 0) {
      const delayDebounceFn = setTimeout(async () => {
        try {
          const _estimate = await getLpEstimation(amount);
          setEstimation(_estimate);
        } catch (error) {
          console.error("Error fetching LP estimation:", error);
        } finally {
          setLoadCalculating(false);
        }
      }, 500); // 500ms debounce delay

      // Cleanup function to clear the timeout if `amount` changes
      return () => clearTimeout(delayDebounceFn);
    } else {
      setEstimation("");
      setLoadCalculating(false);
    }
  }, [amount]);
  return (
    <div className="grid grid-cols-1 md:grid-cols-[65%_34%] gap-3">
      <div className="flex flex-col gap-5 justify-between">
        <div className="flex items-end gap-3 ">
          {globalLoader ? (
            <>
              <div className="h-8 w-24 bg-gray-300 animate-pulse rounded"></div>
              <div className="h-4 w-12 bg-gray-300 animate-pulse rounded opacity-60"></div>
            </>
          ) : (
            <>
              <h1 className="text-3xl font-bold">
                ${Number(assetPrices?.stayPrice)?.toFixed(5)}
              </h1>
              <div className="flex items-center gap-1">
                <div className="flex items-center gap-1">
                  <div className="flex items-center gap-1">
                    <span
                      className="text-xs"
                      style={{ color: percentChange?.lineColor }}
                    >
                      $
                      {String(
                        percentChange?.price_change_24h?.toFixed(4)|| 0
                      ).replace(/[+|-]/g, "") }
                    </span>
                    {percentChange?.price_change_24h < 0 ? (
                      // Down Arrow
                      <div
                        className="w-0 h-0 border-l-4 border-r-4 border-t-4 border-l-transparent border-r-transparent"
                        style={{ borderTopColor: percentChange?.lineColor }}
                      ></div>
                    ) : (
                      // Up Arrow
                      <div
                        className="w-0 h-0 border-l-4 border-r-4 border-b-4 border-l-transparent border-r-transparent"
                        style={{ borderBottomColor: percentChange?.lineColor }}
                      ></div>
                    )}
                    <span
                      className="text-xs"
                      style={{ color: percentChange?.lineColor }}
                    >
                      ({percentChange?.price_change_percentage_24h || 0}%)
                    </span>
                  </div>
                </div>
              </div>
            </>
          )}
        </div>

        {/* Graph Section */}
        {globalLoader ? (
          <div className="h-64 flex items-center justify-center  rounded-lg mt-4 relative">
            <div className="w-7 h-7 border-4 border-t-4 border-t-[#3d4a6c] rounded-full animate-spin"></div>
          </div>
        ) : (
          <Graph />
        )}
        <div className="flex flex-wrap gap-5 justify-between p-4 md:px-12 rounded-xl border shadow">
          <div className=" flex flex-col gap-2">
            <div className="flex items-center gap-2">
              <Image
                src={stay}
                width={50}
                height={50}
                className="size-[1.7rem] -ml-1"
                alt="Stay"
              />
              {globalLoader ? (
                <div className="animate-pulse bg-gray-300 h-6 w-24 rounded"></div>
              ) : (
                <h3 className="text-lg font-bold">
                  {formatNumber(Number(assetPrices.stayReserve))}
                </h3>
              )}
            </div>
            <div className="flex items-center gap-2">
              <Image
                src={usdt}
                width={35}
                height={35}
                className="size-5"
                alt="USDT"
              />
              <h3 className="text-lg font-bold">
                {globalLoader ? (
                  <div className="animate-pulse bg-gray-300 h-6 w-24 rounded"></div>
                ) : (
                  formatNumber(Number(assetPrices.usdcReserve))
                )}
              </h3>
            </div>
          </div>
          <div className=" flex flex-col gap-2">
            <h3 className="text-xl font-bold 2xl:text-3xl">
              {globalLoader ? (
                <div className="animate-pulse bg-gray-300 h-6 w-24 rounded"></div>
              ) : (
                `$ ${formatNumber(Number(assetPrices.usdcReserve * 2))}`
              )}
            </h3>
            <p className="opacity-80 2xl:text-lg">Total value locked</p>
          </div>
          <div className=" flex flex-col gap-2">
            <h3 className="text-xl font-bold 2xl:text-3xl">
              {" "}
              {globalLoader ? (
                <div className="animate-pulse bg-gray-300 h-6 w-24 rounded"></div>
              ) : (
                `$${tradingVolume}`
              )}
            </h3>
            <p className="opacity-80 2xl:text-lg">24h volume</p>
          </div>
          {/* <div className=" flex flex-col gap-2">
            <h3 className="text-xl font-bold 2xl:text-3xl">3.91%</h3>
            <p className="opacity-80 2xl:text-lg">APR</p>
          </div> */}
        </div>
      </div>
      <div className="border shadow-sm rounded-lg p-4 flex flex-col gap-5">
        <h1 className="text-xl font-bold">Provide Liquidity</h1>
        <div className="flex items-center justify-between">
          <div className="flex items-start  gap-5">
            <span className="text-xl font-bold">1. </span>
            <button
              onClick={() => setCurrentButton("add")}
              type="button"
              className="flex flex-col w-max"
            >
              <h2
                className={`text-xl transition-all font-bold ${
                  currentButton == "add" ? "opacity-100" : "opacity-50"
                } `}
              >
                Add
              </h2>
              {currentButton == "add" ? (
                <div className="w-10 h-1 bg-[#954AFC]"></div>
              ) : null}
            </button>
            <button
              onClick={() => setCurrentButton("remove")}
              type="button"
              className="flex flex-col w-max"
            >
              <h2
                className={`text-lg font-bold transition-all  ${
                  currentButton == "remove" ? "opacity-100" : "opacity-50"
                }`}
              >
                Remove
              </h2>
              {currentButton == "remove" ? (
                <div className="w-full h-1 bg-[#954AFC]"></div>
              ) : null}
            </button>
          </div>
          {/* <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                        <path d="M19.8999 12.6599C19.7396 12.4774 19.6512 12.2428 19.6512 11.9999C19.6512 11.757 19.7396 11.5224 19.8999 11.3399L21.1799 9.89989C21.3209 9.74256 21.4085 9.5446 21.4301 9.3344C21.4516 9.12421 21.4061 8.91258 21.2999 8.72989L19.2999 5.2699C19.1948 5.08742 19.0348 4.94277 18.8426 4.85658C18.6505 4.77039 18.4361 4.74705 18.2299 4.7899L16.3499 5.1699C16.1107 5.21932 15.8616 5.17948 15.6498 5.0579C15.4379 4.93631 15.2779 4.74138 15.1999 4.5099L14.5899 2.6799C14.5228 2.48127 14.395 2.30876 14.2245 2.18674C14.0541 2.06471 13.8495 1.99935 13.6399 1.9999H9.6399C9.42183 1.98851 9.20603 2.04882 9.02546 2.17161C8.84489 2.2944 8.70948 2.47291 8.6399 2.6799L8.0799 4.5099C8.0019 4.74138 7.84187 4.93631 7.63001 5.0579C7.41815 5.17948 7.16911 5.21932 6.9299 5.1699L4.9999 4.7899C4.80445 4.76228 4.6052 4.79312 4.42724 4.87853C4.24929 4.96395 4.1006 5.10012 3.9999 5.2699L1.9999 8.72989C1.89106 8.91054 1.84212 9.12098 1.86008 9.33112C1.87804 9.54126 1.96198 9.74034 2.0999 9.89989L3.3699 11.3399C3.53022 11.5224 3.61863 11.757 3.61863 11.9999C3.61863 12.2428 3.53022 12.4774 3.3699 12.6599L2.0999 14.0999C1.96198 14.2595 1.87804 14.4585 1.86008 14.6687C1.84212 14.8788 1.89106 15.0892 1.9999 15.2699L3.9999 18.7299C4.10499 18.9124 4.26502 19.057 4.45715 19.1432C4.64928 19.2294 4.86372 19.2527 5.0699 19.2099L6.9499 18.8299C7.18911 18.7805 7.43815 18.8203 7.65001 18.9419C7.86187 19.0635 8.0219 19.2584 8.0999 19.4899L8.7099 21.3199C8.77948 21.5269 8.91489 21.7054 9.09546 21.8282C9.27603 21.951 9.49183 22.0113 9.7099 21.9999H13.7099C13.9195 22.0004 14.1241 21.9351 14.2945 21.8131C14.465 21.691 14.5928 21.5185 14.6599 21.3199L15.2699 19.4899C15.3479 19.2584 15.5079 19.0635 15.7198 18.9419C15.9316 18.8203 16.1807 18.7805 16.4199 18.8299L18.2999 19.2099C18.5061 19.2527 18.7205 19.2294 18.9126 19.1432C19.1048 19.057 19.2648 18.9124 19.3699 18.7299L21.3699 15.2699C21.4761 15.0872 21.5216 14.8756 21.5001 14.6654C21.4785 14.4552 21.3909 14.2572 21.2499 14.0999L19.8999 12.6599ZM18.4099 13.9999L19.2099 14.8999L17.9299 17.1199L16.7499 16.8799C16.0297 16.7327 15.2805 16.855 14.6445 17.2237C14.0085 17.5924 13.53 18.1817 13.2999 18.8799L12.9199 19.9999H10.3599L9.9999 18.8599C9.76975 18.1617 9.29128 17.5724 8.6553 17.2037C8.01932 16.835 7.27012 16.7127 6.5499 16.8599L5.3699 17.0999L4.0699 14.8899L4.8699 13.9899C5.36185 13.4399 5.63383 12.7278 5.63383 11.9899C5.63383 11.252 5.36185 10.5399 4.8699 9.98989L4.0699 9.0899L5.3499 6.88989L6.5299 7.1299C7.25012 7.27712 7.99932 7.15478 8.6353 6.78609C9.27128 6.41741 9.74975 5.82805 9.9799 5.1299L10.3599 3.9999H12.9199L13.2999 5.13989C13.53 5.83805 14.0085 6.42741 14.6445 6.79609C15.2805 7.16478 16.0297 7.28712 16.7499 7.13989L17.9299 6.8999L19.2099 9.11989L18.4099 10.0199C17.9235 10.5687 17.6549 11.2766 17.6549 12.0099C17.6549 12.7432 17.9235 13.4511 18.4099 13.9999ZM11.6399 7.9999C10.8488 7.9999 10.0754 8.23449 9.41761 8.67402C8.75982 9.11354 8.24713 9.73826 7.94438 10.4692C7.64163 11.2001 7.56241 12.0043 7.71675 12.7803C7.8711 13.5562 8.25206 14.2689 8.81147 14.8283C9.37088 15.3877 10.0836 15.7687 10.8595 15.923C11.6355 16.0774 12.4397 15.9982 13.1706 15.6954C13.9015 15.3927 14.5262 14.88 14.9658 14.2222C15.4053 13.5644 15.6399 12.791 15.6399 11.9999C15.6399 10.939 15.2185 9.92161 14.4683 9.17147C13.7182 8.42132 12.7008 7.9999 11.6399 7.9999ZM11.6399 13.9999C11.2443 13.9999 10.8577 13.8826 10.5288 13.6628C10.1999 13.4431 9.94351 13.1307 9.79214 12.7653C9.64076 12.3998 9.60116 11.9977 9.67833 11.6097C9.7555 11.2218 9.94598 10.8654 10.2257 10.5857C10.5054 10.306 10.8618 10.1155 11.2497 10.0383C11.6377 9.96115 12.0398 10.0008 12.4053 10.1521C12.7707 10.3035 13.0831 10.5599 13.3028 10.8888C13.5226 11.2177 13.6399 11.6043 13.6399 11.9999C13.6399 12.5303 13.4292 13.039 13.0541 13.4141C12.679 13.7892 12.1703 13.9999 11.6399 13.9999Z" fill="black" />
                    </svg> */}
        </div>
        {currentButton == "add" ? (
          <>
            <div className="flex flex-col gap-2">
              <h3 className="font-medium text-lg">Amount in USDC</h3>
              <div className="bg-[#F5F5F5] w-full flex items-center rounded-lg border">
                <div className="flex items-center gap-1 w-full pl-3">
                  <span className="text-lg font-semibold">$</span>
                  <input
                    disabled={isLoading}
                    type="number"
                    min={0}
                    placeholder="0"
                    className="border-none bg-transparent w-full p-4 py-2 pl-0 outline-none text-lg font-semibold"
                    value={amount}
                    onChange={(e) => {
                      setAmount(e.target.value);
                    }}
                  />
                </div>
              </div>
            </div>
            <div className="flex flex-col gap-2">
              <h3 className="font-medium text-lg">LP Quantity</h3>
              <div className="bg-[#F5F5F5] w-full flex items-center rounded-lg border py-2">
                <div className="flex items-center gap-1 w-full pl-3">
                  <div className="flex items-center justify-between">
                    {loadCalculating ? (
                      // Skeleton Loader
                      <div className="w-20 h-5 bg-gray-300 rounded animate-pulse"></div>
                    ) : (
                      // Estimation Value
                      // <h1
                      //   className={`font-bold text-base ${
                      //     amount == 0 ? "opacity-20" : "opacity-100"
                      //   } transition-all `}
                      // >
                      //   {estimation}
                      // </h1>

                      // <input
                      //   type="number"
                      //   placeholder="0"
                      //   className={`font-bold text-base text-black ${
                      //     amount == 0 ? "opacity-20" : "opacity-100"
                      //   } transition-all bg-transparent border-none outline-none`}
                      //   value={amount > 0 ? estimation : ""}
                      //   readOnly
                      // />

                      <input
                        disabled={isLoading}
                        type="number"
                        placeholder="0"
                        className="border-none bg-transparent w-full p-4 py-0 pl-0 outline-none text-lg font-semibold"
                        value={amount > 0 ? estimation : ""}
                        readOnly
                      />
                    )}
                  </div>
                </div>
                <span className="flex items-center gap-1 font-bold mr-4 text-base 2xl:text-lg">
                  <div className="flex items-center -space-x-3 mr-1">
                    <Image
                      src={usdt}
                      width={60}
                      height={60}
                      className="shrink-0 size-5"
                      alt="LP Token"
                    />
                    <Image
                      src={stay}
                      width={60}
                      height={60}
                      className="shrink-0 size-7"
                      alt="LP Token"
                    />
                  </div>
                  LP
                </span>
              </div>
            </div>
            <div className="flex items-center gap-3 flex-wrap w-full justify-between">
              <button
                disabled={
                  isLoading ||
                  !amount ||
                  amount <= 0 ||
                  loadCalculating ||
                  isCardLoading
                }
                onClick={() => handleLpBuy(connectedAddress)}
                className="border-2 disabled:opacity-55 disabled:cursor-not-allowed border-[#954AFC] text-[#954AFC] px-4 py-1.5 rounded-lg font-semibold text-[12px] 2xl:text-lg flex items-center justify-center gap-2 w-fit" // Fixed width
              >
                {/* Conditionally render loader or button text */}
                {isLoading ? (
                  <>
                    <div className="w-5 h-5 border-4 border-t-4 border-t-[#954AFC] border-[#f0f0f0] rounded-full animate-spin"></div>{" "}
                    {isBuyLpApprovalLoading ? "Approval Pending (1/2)" : "Buying LP (2/2)"}
                  </> // Loader
                ) : (
                  "Crypto Payment" // Button Text
                )}
              </button>

              <button
                disabled={
                  isLoading ||
                  !amount ||
                  amount <= 0 ||
                  loadCalculating ||
                  isCardLoading
                }
                onClick={() => buyLpWithCard(amount)}
                className="disabled:bg-opacity-55 disabled:cursor-not-allowed  bg-[#954AFC] text-white px-4 py-2 rounded-lg flex items-center gap-1 font-semibold text-[12px]  2xl:text-lg"
              >
                <svg
                  width="13"
                  height="13"
                  className="2xl:size-6"
                  viewBox="0 0 13 13"
                  fill="none"
                  xmlns="http://www.w3.org/2000/svg"
                >
                  <path
                    d="M11.9171 6.50008C11.9171 4.45746 11.9171 3.43587 11.2822 2.80158C10.6474 2.16729 9.62636 2.16675 7.58374 2.16675H5.41707C3.37445 2.16675 2.35287 2.16675 1.71857 2.80158C1.08428 3.43641 1.08374 4.45746 1.08374 6.50008C1.08374 8.54271 1.08374 9.56429 1.71857 10.1986C2.35341 10.8329 3.37445 10.8334 5.41707 10.8334H7.58374C9.62636 10.8334 10.6479 10.8334 11.2822 10.1986C11.6365 9.84487 11.793 9.37091 11.8618 8.66675M5.41707 8.66675H3.25041M7.58374 8.66675H6.77124M1.08374 5.41675H3.79207M11.9171 5.41675H5.95874"
                    stroke="white"
                    stroke-width="0.8125"
                    stroke-linecap="round"
                  />
                </svg>
                Card Payment
              </button>
            </div>
            <div className="flex items-start gap-5">
              <span className="text-xl font-bold">2. </span>
              <div className="flex flex-col w-max">
                {globalLoader ? (
                  <div className="animate-pulse bg-gray-300 h-8 w-48 rounded"></div>
                ) : (
                  <h2 className="text-xl font-bold">
                    Stake {formatNumber(Number(assetBalance.lpBalance))} LP
                  </h2>
                )}
              </div>
            </div>

            <button
              onClick={() => handleAddLiqudity(connectedAddress)}
              disabled={
                isAddFarmLoading ||
                isLoading ||
                Number(assetBalance?.lpBalance) == 0
              }
              className="disabled:bg-opacity-55 disabled:cursor-not-allowed bg-[#954AFC] text-white px-4 py-3 font-semibold text-sm text-center rounded-lg flex items-center justify-center gap-1 2xl:text-lg"
            >
              {isAddFarmLoading ? (
                // Loader (spinner)
                <>
                  {" "}
                  <div className="w-5 h-5 border-4 border-t-4 border-white rounded-full animate-spin"></div>{" "}
                  {isFarmApprovalLoading
                    ? "Approval Pending (1/2)"
                    : "Adding to farm (2/2)"}
                </>
              ) : (
                // Button Text
                "Add to Farm"
              )}
            </button>

            <div className="flex items-start  gap-5">
              <span className="text-xl font-bold">3. </span>
              <div className="flex flex-col w-max">
                <h2 className="text-xl font-bold">Earn</h2>
              </div>
            </div>
            <div className="flex flex-col gap-2">
              <div className="flex items-center justify-between gap-3">
                <h3 className="font-medium text-lg">Stay Earned</h3>
                <Image
                  src={stay}
                  width={60}
                  height={60}
                  className="shrink-0 size-5"
                  alt="LP Token"
                />
              </div>
              <div className="bg-[#F5F5F5] w-full flex items-center rounded-lg border">
                <div className="flex items-center gap-1 w-full pl-3">
                  <span className="text-lg font-semibold"></span>
                  <input
                    readOnly
                    type="number"
                    placeholder="0"
                    value={userDetails.pendingRewards.toFixed(3)}
                    className="border-none bg-transparent w-full p-4 pl-0 outline-none text-lg font-semibold"
                  />
                </div>
              </div>
              <div className="flex items-center gap-3 flex-wrap w-full justify-between font-medium">
                <button
                  disabled={
                    isClaimLoading || Number(userDetails.pendingRewards) == 0
                  }
                  onClick={handleOpen}
                  className=" disabled:bg-opacity-55 disabled:cursor-not-allowed bg-[#954AFC] text-white px-4 py-2 rounded-lg flex items-center gap-1 font-semibold text-sm 2xl:text-lg"
                >
                  Claim & Reinvest
                </button>
                <button
                  disabled={
                    isClaimLoading || Number(userDetails.pendingRewards) == 0
                  }
                  onClick={() => claimRewards(connectedAddress)}
                  className="relative disabled:bg-opacity-55 disabled:cursor-not-allowed border px-4 py-2 rounded-lg font-semibold text-sm 2xl:text-lg w-36"
                >
                  {/* Button Text */}
                  {!isClaimLoading && "Claim"}

                  {/* Loader */}
                  {isClaimLoading && (
                    <div className="flex justify-center items-center">
                      <div className="w-5 h-5 border-4 border-t-4 border-[#954AFC] rounded-full animate-spin"></div>
                    </div>
                  )}
                </button>
              </div>
            </div>
          </>
        ) : (
          <>
            <div className="flex flex-col gap-3">
              <div className="flex items-center font-semibold text-base gap-3">
                <span className="flex items-center gap-1 font-bold mr-4 text-base 2xl:text-lg">
                  <div className="flex items-center -space-x-3 mr-1">
                    <Image
                      src={usdt}
                      width={60}
                      height={60}
                      className="shrink-0 size-5"
                      alt="LP Token"
                    />
                    <Image
                      src={stay}
                      width={60}
                      height={60}
                      className="shrink-0 size-7 "
                      alt="LP Token"
                    />
                  </div>
                  LP
                </span>
                <span>LP Staked</span>
                <span>{Number(userDetails.staked).toFixed(2)}</span>
              </div>
              <div className="bg-[#F5F5F5] w-full flex items-center rounded-lg border">
                <div className="flex items-center gap-1 w-full px-3">
                  {/* <span className="text-lg font-semibold">$</span> */}
                  <input
                    disabled={isRemoveLoading || !userDetails.staked}
                    type="number"
                    min={0}
                    placeholder="0"
                    value={balance || ""}
                    onChange={(e) =>
                      e.target.value > -1 && setBalance(e.target.value)
                    }
                    className="border-none  disabled:opacity-50 bg-transparent w-full p-4 pl-0 outline-none text-lg font-semibold"
                  />
                  <button
                    disabled={
                      isRemoveLoading || Number(userDetails.staked) === 0
                    }
                    type="button"
                    onClick={() => setBalance(userDetails.staked)}
                    className="disabled:bg-opacity-55 disabled:cursor-not-allowed px-3 py-1 border-2 border-black"
                  >
                    Max
                  </button>
                </div>
              </div>
              <div className="flex flex-col gap-2">
                <h3 className="font-medium text-lg">Amount in USD <Tooltip
  text={`Reminder: After withdrawing, you’ll receive your LP tokens back. To access your USDC and STAY tokens, you’ll still need to remove liquidity manually on PancakeSwap.`}
  className="text-xs"
/></h3>
                <div className="bg-[#F5F5F5] w-full flex items-center rounded-lg border">
                  <div className="flex items-center gap-2 w-full p-3 py-5">
                    {/* <Image
                      src={usdt}
                      width={60}
                      height={60}
                      className="shrink-0 size-5 "
                      alt="LP Token"
                    /> */}
                    <span className="font-bold text-base">
                      $
                      {(balance * assetPrices.lpPrice).toLocaleString({
                        maximumFractionDigits: 2,
                        minimumFractionDigits: 2,
                      }) || 0}
                    </span>
                  </div>
                </div>
              </div>
              <button
                onClick={() => handleRemoveLiquidity(balance)}
                disabled={
                  isRemoveLoading ||
                  !balance ||
                  balance > Number(userDetails.staked) ||
                  userDetails.staked === 0
                }
                className="disabled:bg-opacity-55 disabled:cursor-not-allowed border-2 border-[#954AFC] text-[#954AFC] px-4 py-2 rounded-lg font-semibold text-[12px] 2xl:text-lg w-fit"
              >
                <div className="relative w-full h-full flex justify-center items-center">
                  {/* Button Text */}
                  <span
                    className={`${
                      isRemoveLoading ? "opacity-0" : "opacity-100"
                    } transition-opacity`}
                  >
                    Remove
                  </span>

                  {/* Loader (spinner) */}
                  {isRemoveLoading && (
                    <div className="absolute flex justify-center items-center">
                      <div className="w-4 h-4 border-4 border-t-4 border-[#954AFC] rounded-full animate-spin"></div>
                    </div>
                  )}
                </div>
              </button>
            </div>
          </>
        )}
      </div>
      <Claim
        source="farm"
        setIsSuccess={setIsSuccess}
        steps={steps}
        open={open}
        setOpen={setOpen}
      />
      <Congratulation
        open={isSuccess}
        handleClose={() => {
          setIsSuccess(false);
        }}
      />
    </div>
  );
};

export default Analytics;
