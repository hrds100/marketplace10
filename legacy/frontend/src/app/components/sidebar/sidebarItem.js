...
const SidebarItem = ({ item, pageName, setPageName }) => {
    ...
    const isActive = (item) => {
        ...
        if (pathname.includes(item.route)) return true;
        if (item.children) {
            ...
            <li>
                ...
                <Link
                    ...
                    rel="noopener noreferrer" target={
                        item.route.includes("http") ? "_blank" : "_self"
                    }
                    onClick={handleClick}
                    className={`${isItemActive ? "bg-[#edecff] " : ""} group relative flex items-center 2xl:text-lg gap-2.5 rounded-sm px-4 py-2 font-medium text-[#0C0839] duration-300 ease-in-out hover:bg-[#edecff] `}
                >
