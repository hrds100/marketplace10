...
| Phase | Status | Notes |
|-------|--------|-------|
| Phase 0 — Lovable UI Build | READY | Prompt at docs/LOVABLE_PROMPT.md (2,361 lines). Paste into Lovable, download project, copy hooks. |
| Phase 1 — Foundation | UI BUILT BY LOVABLE | Run DB migrations + copy 20 hooks from src/hooks/nfstay/ |
| Phase 2 — Properties + Maps | UI BUILT BY LOVABLE | Run DB migrations + Storage buckets |
| Phase 3 — Reservations + Pricing | UI BUILT BY LOVABLE | Deploy nfs-email-send |
| Phase 4 — Payments (Stripe) | UI BUILT BY LOVABLE | Deploy Edge Functions + Stripe keys (Tajul has access) |
| Phase 5 — Integrations (Hospitable + iCal) | UI BUILT BY LOVABLE | Hospitable credentials may need refresh |
| Phase 6 — White-Label + Analytics | UI BUILT BY LOVABLE | DNS + domain config + Cloudflare |

> **IMPORTANT (2026-03-20):** The NFStay UI is being built by Lovable AI, not manually.
> The prompt is at `docs/LOVABLE_PROMPT.md`. Do NOT manually rebuild pages Lovable generates.
> Tajuul's 20 hooks in `src/hooks/nfstay/` are complete and must be copied into the Lovable project.
>
> **WARNING:** The branch `claude/design-agent-prompts-0Fd0G` contains invest module modifications that must NOT be merged. Use hooks from this branch but do NOT merge the branch itself.
> **Decision (2026-03-19):** NFStay uses shared signup — no separate NfsOperatorSignup page. Activation via onboarding wizard.

---

## PHASE 0 — LOVABLE UI BUILD

**Goal:** Get the complete NFStay UI generated by Lovable AI.
**Dependencies:** None — do this first.
**Tajuul needed:** Yes — paste prompt into Lovable, download project, copy hooks.

### Step 0.1 — Generate UI in Lovable

1. Go to [lovable.dev](https://lovable.dev) and create a new project
2. Open `docs/LOVABLE_PROMPT.md` — copy the entire contents (everything after the first line)
3. Paste into Lovable's prompt field
4. Set Supabase project: `asazddtvjvmckouxcmmo`
5. Let Lovable build — this generates all 30+ pages, all layouts, all modals

**What Lovable builds:** Every page, button, form, modal, and layout from the 2,361-line spec. Green design system, Inter font, full operator/traveler/admin/white-label portals.

---

### Step 0.2 — Download and set up the project

1. Export/download the Lovable project (GitHub export or ZIP)
2. Clone/open in your code editor
3. Run `npm install`
4. Verify `npm run dev` starts on port 8080

---

### Step 0.3 — Copy the 20 pre-built hooks

Copy ALL files from `marketplace10/src/hooks/nfstay/` into the Lovable project's equivalent hooks directory:
...
use-nfs-reservation.ts
use-nfs-pricing.ts
use-nfs-analytics.ts
use-nfs-promo-codes.ts
use-nfs-availability.ts
use-nfs-white-label.ts
use-nfs-google-maps.ts
use-nfs-property-mutation.ts
use-nfs-reservation-mutation.ts
use-nfs-operator-update.ts
use-nfs-property-wizard.ts
use-nfs-image-upload.ts
use-nfs-stripe.ts
use-nfs-hospitable.ts
use-nfs-onboarding.ts
```

These replace Lovable's auto-generated data-fetching stubs with the real Supabase queries.

---

### Step 0.4 — Copy component stubs

Copy from `marketplace10/src/components/nfstay/` any components that have working logic (maps, booking widget, photo gallery, wizard steps, settings tabs). Let Lovable's UI shell wrap them.

---

### Step 0.5 — Verify zero TypeScript errors

Run: `npm run check`

Fix any import path mismatches between Lovable's structure and the copied hooks.

---

### Step 0.6 — Phase 0 verification

- [ ] Lovable project runs locally with no blank screens
- [ ] All 30+ routes render (even with placeholder data)
- [ ] Green design system applied throughout
- [ ] 20 hooks copied and imported correctly
- [ ] TypeScript: zero errors
- [ ] Auth sign-in/sign-up routes work via Supabase

**Done when:** The app looks complete visually and compiles clean. Data is placeholder/empty until DB migrations run (Phase 1+).
